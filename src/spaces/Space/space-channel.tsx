// path: src/spaces/components/Space/space-channel.tsx
"use client";
...
import { Channel as PayloadChannel, Space } from '@/payload-types';

// Define a type that extends PayloadChannel with our additional properties
interface ExtendedChannel extends Omit<PayloadChannel, 'type'> {
  type: ChannelType;
  spaceId: string;
}
...
export const SpaceChannel = ({
  ...
  const onClick = () => {
    ...
    router.push(`/${CollectionSlugs.SPACES}/${params?.spaceId}/${CollectionSlugs.CHANNELS}/${channel.id}`)
  }
