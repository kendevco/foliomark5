...
import { Member, Message } from "@/spaces/collections/types";
import { useChatQuery } from "@/spaces/hooks/use-chat-query";
import { useChatSocket } from "@/spaces/hooks/use-chat-socket";
...
export const ChatMessages = ({
  ...
  const { ref, messages } = useInfiniteScroll({
    hasMore: !!hasNextPage,
    ...
    <div ref={chatRef} className="flex-1 flex flex-col py-4 overflow-y-auto">
      ...
      <div className="flex flex-col-reverse mt-auto">
        {data?.pages?.map((group, i) => (
          <Fragment key={i}>
            {group.docs.map((message: Message) => (
              <ChatItem
                key={message.id}
                id={message.id}
