...
export const InviteModal = () => {
    ...
    const inviteUrl = `${origin}/invite/${(space as any)?.inviteCode}`;
    ...
    const onNew = async () => {
        try {
            setIsLoading(true);
            const response = await axios.patch(`/api/spaces/${(space as any)?.id}/invite-code`);

            onOpen("invite", { space: response.data });
    ...
    const handleCopy = () => {
        ...
        if (data.space) {
            track('Invite Link Copied', { spaceId: data.space.id });
        }
        onCopy();
    };
